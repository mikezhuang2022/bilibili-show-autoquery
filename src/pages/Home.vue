...
<template>
  ...
  <ul>
    <li v-for="exhibition in exhibitions" :key="exhibition.project_id" class="mb-3 mx-3 md:mx-11 border-b-2 border-b-slate-200 p-2 flex flex-col md:flex-row">
      <div class="grow">
        <p class="text:text-2xl md:text-3xl">{{ exhibition.project_name }}
          <div class="md:inline"
      ...
      <div class="flex-none">
        <p class="text-red-400 text-2xl relative right-0 w-fit md:ml-auto" :class="{ 'text-white': exhibition.price_low === 0 }">￥{{ exhibition.price_low / 100 }}-{{ exhibition.price_high / 100 }}</p>
